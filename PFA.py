...
def show_employees_window():
    ...
    for idx, employee in enumerate(employees):
        ...
        tk.Label(employees_window, text=f"Id:").grid(row=idx, column=0)
        id_entry = tk.Entry(employees_window)
        id_entry.insert(0, employee_id)
        id_entry.grid(row=idx, column=1)


        # Create entry fields for each employee's information
        tk.Label(employees_window, text=f"Nom:").grid(row=idx, column=2)
        nom_entry = tk.Entry(employees_window)
        nom_entry.insert(0, nom)
        nom_entry.grid(row=idx, column=3)

        tk.Label(employees_window, text=f"Prénom:").grid(row=idx, column=4)
        prenom_entry = tk.Entry(employees_window)
        prenom_entry.insert(0, prenom)
        prenom_entry.grid(row=idx, column=5)

        tk.Label(employees_window, text=f"Poste:").grid(row=idx, column=6)
        poste_entry = tk.Entry(employees_window)
        poste_entry.insert(0, poste)
        poste_entry.grid(row=idx, column=7)

        tk.Label(employees_window, text=f"Département:").grid(row=idx, column=8)
        departement_entry = tk.Entry(employees_window)
        departement_entry.insert(0, departement)
        departement_entry.grid(row=idx, column=9)
        ...
        update_button.grid(row=idx, column=10)
